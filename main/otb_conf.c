...
void otb_conf_ads_init_one(otb_conf_ads *ads, char ii)
{
  ...
  memset(ads, 0, sizeof(otb_conf_ads));
  ads->index = ii;
  ads->mux = 0b000;
  ads->gain = 0b010;
  ads->rate = 0b100;
...
void otb_conf_ads_init(otb_conf_struct *conf)
{
  ...
  // Belt and braces:
  memset(conf->ads, 0, OTB_CONF_ADS_MAX_ADSS * sizeof(otb_conf_ads));
...
bool otb_conf_verify(otb_conf_struct *conf)
{
  ...
  if (!rc)
  {
    ...
    {
      memset((void *)&(conf->ip), 0, sizeof(conf->ip));
      conf->mqtt_httpd = OTB_CONF_MQTT_HTTPD_DISABLED;
      memset((void *)(conf->pad4), 0, 3);
      modified = TRUE;
  ...
  {
    if ((strnlen(conf->ssid, OTB_CONF_WIFI_SSID_MAX_LEN) >=
                                                            OTB_CONF_WIFI_SSID_MAX_LEN) ||
        (strnlen(conf->password, OTB_CONF_WIFI_PASSWORD_MAX_LEN) >=
                                                          OTB_CONF_WIFI_PASSWORD_MAX_LEN))
    {
      ...
      memset(conf->ssid, 0, OTB_CONF_WIFI_SSID_MAX_LEN);
      memset(conf->password, 0, OTB_CONF_WIFI_PASSWORD_MAX_LEN);
      modified = TRUE;
    }
    ...
    if (conf->ds18b20s > OTB_DS18B20_MAX_DS18B20S)
    {
      ...
      conf->ds18b20s = 0;
      memset(conf->ds18b20, 0, OTB_DS18B20_MAX_DS18B20S * sizeof(otb_conf_ds18b20));
    }
  
    for (ii = 0; ii < OTB_DS18B20_MAX_DS18B20S; ii++)
    {
      // Check DS18B20 id is right length (or 0), and location is null terminated
      if (((strnlen(conf->ds18b20[ii].id, OTB_CONF_DS18B20_MAX_ID_LEN) !=
                                                         OTB_CONF_DS18B20_MAX_ID_LEN-1) &&
           (conf->ds18b20[ii].id[0] != 0)) ||
          (strnlen(conf->ds18b20[ii].loc, OTB_CONF_DS18B20_LOCATION_MAX_LEN) >=
                                                       OTB_CONF_DS18B20_LOCATION_MAX_LEN))
      {
        MWARN("DS18B20 index %d id or location invalid", ii);
        memset(conf->ds18b20[ii].id, 0, OTB_CONF_DS18B20_MAX_ID_LEN);
        memset(conf->ds18b20[ii].loc, 0, OTB_CONF_DS18B20_LOCATION_MAX_LEN);
        modified = TRUE;
      }
    }
  
    if ((strnlen(conf->loc.loc1, OTB_CONF_LOCATION_MAX_LEN) >=
                                                             OTB_CONF_LOCATION_MAX_LEN) ||
        (strnlen(conf->loc.loc2, OTB_CONF_LOCATION_MAX_LEN) >=
                                                             OTB_CONF_LOCATION_MAX_LEN) ||
        (strnlen(conf->loc.loc3, OTB_CONF_LOCATION_MAX_LEN) >=
                                                               OTB_CONF_LOCATION_MAX_LEN))

    {
      MWARN("Location invalid");
      memset(conf->loc.loc1, 0, OTB_CONF_LOCATION_MAX_LEN);
      memset(conf->loc.loc2, 0, OTB_CONF_LOCATION_MAX_LEN);
      memset(conf->loc.loc3, 0, OTB_CONF_LOCATION_MAX_LEN);
      modified = TRUE;
    }
    ...
    for (ii = 0; ii < OTB_CONF_ADS_MAX_ADSS; ii++)
    {
      ...
      if ((strnlen(conf->ads[ii].loc, OTB_CONF_ADS_LOCATION_MAX_LEN) >=
          ...
          (conf->ads[ii].cont > 1) ||
          (conf->ads[ii].rms > 1) ||
          (conf->ads[ii].period >= 0xffff) ||
          (conf->ads[ii].samples >= 1024))
      {
    ...
    for (ii = 0; ii < OTB_CONF_RELAY_MAX_MODULES; ii++)
    {
      if ((strnlen(conf->relay[ii].loc, OTB_CONF_RELAY_LOC_LEN) >=
          ...
          (conf->relay[ii].type >= OTB_CONF_RELAY_TYPE_NUM) ||
          (conf->relay[ii].num_relays > OTB_CONF_RELAY_MAX_PER_MODULE) ||
          (conf->relay[ii].pad1[0] != 0) ||
          (conf->relay[ii].pad1[1] != 0) ||
          (conf->relay[ii].pad1[2] != 0) ||
          ((conf->relay[ii].type == OTB_CONF_RELAY_TYPE_OTB_0_4) &&
           ((conf->relay[ii].addr > 7) ||
          ...
          ((conf->relay[ii].type == OTB_CONF_RELAY_TYPE_PCA) &&
           ((conf->relay[ii].addr < 0x40) ||
            (conf->relay[ii].addr > 0x3f) ||
            (conf->relay[ii].num_relays > 16) ||
            (conf->relay[ii].status_led < -1) ||
            (conf->relay[ii].status_led > 16))))
      {
        ...
        memset(&(conf->relay[ii]), 0, sizeof(otb_conf_relay));
        conf->relay[ii].index = ii;
        modified = TRUE;
      }
    }    
...
uint8 otb_conf_store_sta_conf(char *ssid, char *password, bool commit)
{
  ...
  if (strncmp(conf->ssid, ssid, OTB_CONF_WIFI_SSID_MAX_LEN) ||
      strncmp(conf->password, password, OTB_CONF_WIFI_PASSWORD_MAX_LEN))
  {
    MDETAIL("New config: ssid, password");
    rc = OTB_CONF_RC_CHANGED;
    strncpy(conf->ssid, ssid, OTB_CONF_WIFI_SSID_MAX_LEN);
    strncpy(conf->password, password, OTB_CONF_WIFI_PASSWORD_MAX_LEN);
    if (commit)
    {
...
bool otb_conf_update_loc(int loc, char *val)
{
  ...
  {
    case 1:
      strncpy(conf->loc.loc1, val, OTB_CONF_LOCATION_MAX_LEN);
      break;
  
    case 2:
      strncpy(conf->loc.loc2, val, OTB_CONF_LOCATION_MAX_LEN);
      break;
  
    case 3:
      strncpy(conf->loc.loc3, val, OTB_CONF_LOCATION_MAX_LEN);
      break;
  ...
  else if (otb_mqtt_match(cmd1, OTB_MQTT_CMD_GET))
  {
    ...
    switch (field)
    {
      ...
      case OTB_MQTT_CONFIG_DS18B20S_:
        snprintf(response, 8, "%d", otb_conf->ds18b20s);
        otb_mqtt_send_status(OTB_MQTT_SYSTEM_CONFIG,
                             OTB_MQTT_CMD_GET,
                             OTB_MQTT_STATUS_OK,
                             response);
      ...
      case OTB_MQTT_CONFIG_ALL_:
        {
          ...
          unsigned char kk = 0;
          snprintf(scratch, 128, "len:%d", sizeof(otb_conf_struct));
          otb_mqtt_send_status(OTB_MQTT_SYSTEM_CONFIG,
                               OTB_MQTT_CMD_GET,
                               OTB_MQTT_STATUS_OK,
                               scratch);
          ...
          {
            snprintf(scratch+(kk*2), (128-(kk*2)), "%02x", conf_struct[jj]);
            if (kk >= 63)
            {
