...
{
  bool success = FALSE;
  bool i2c_driver_installed = FALSE;
  ...
  info->mode = I2C_MODE_MASTER;
  info->sda_io_num = sda_pin;
  info->scl_io_num = scl_pin;
  info->sda_pullup_en = 0; // Have hardware pullups
  info->scl_pullup_en = 0; // Have hardware pullups
  info->clk_stretch_tick = 300; // 300 ticks ~= 210us

  ESP_ERR_WARN_AND_GOTO_EXIT_LABEL(success, i2c_driver_install(I2C_NUM_0, info->mode));
  i2c_driver_installed = TRUE;
  ESP_ERR_WARN_AND_GOTO_EXIT_LABEL(success, i2c_param_config(I2C_NUM_0, info));
