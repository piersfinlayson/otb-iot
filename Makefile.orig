...
IDF_PATH ?= $(SDK_BASE)/$(RTOS_DIR)
IDF_COMP = $(IDF_PATH)/components
...
ESP_D = -D__ESP_FILE__=__FILE__		
COMMON_FLAGS = -Wno-frame-address \
               -ffunction-sections \
							 -fdata-sections \
							 -fstrict-volatile-bitfields \
							 -mlongcalls \
							 -nostdlib
CFLAGS = $(COMMON_FLAGS) -std=gnu99 -Os -Iinclude -Iinclude/boards $(IDF_INCLUDE) $(ESP_D) -mlongcalls -c -ggdb -Wpointer-arith -Wundef -Wno-address -Wl,-El -fno-inline-functions -mtext-section-literals -DICACHE_FLASH -Werror -D__ets__ -Ilib/rboot $(HW_DEFINES) -Ilib/esp8266-software-uart/softuart/include
...
# Link options
LDLIB_DIRS = -L$(ESP_SDK)/lib \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/esp8266 \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/freertos \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/lwip \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/main \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/esp_common \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/log \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/wpa_supplicant \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/util \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/heap \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/tcpip_adapter \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/nvs_flash \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/libsodium \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/spi_flash \
             -L$(IDF_PATH)/examples/get-started/hello_world/build/newlib \
             -L$(IDF_PATH)/components/newlib/newlib/lib \
						 -Lbin
LDLIBS = $(COMMON_FLAGS) -Wl,--gc-sections -Wl,-static -Wl,--start-group -lc -lgcc -lgcov -lstdc++ -lcore -lwpa_supplicant -lwpa -lnet80211 -lhal -lsmartconfig -lrtc -lwps -lclk -lssc -lwpa2 -lpp -lphy -lesp8266 -llwip -lfreertos -lmain -lesp_common -llog -lutil -lheap -ltcpip_adapter -lnvs_flash -llibsodium -lspi_flash -lnewlib -Wl,--end-group -Wl,-EL
BUILD_NUM_FILE = include/otb_build_num.txt
# XXX RTOS SDK change - prob need to modify ld scripts
LD_DIR = $(ESP_SDK)/ld
LD_MAIN_SCRIPT = $(LD_DIR)/esp8266.ld
LDFLAGS = $(LDLIB_DIRS) -Wl,--no-check-sections -Wl,-static -u call_user_start_cpu0 -u Cache_Read_Enable_New -T$(LD_DIR)/esp8266.rom.ld -T$(LD_DIR)/esp8266.peripherals.ld -T$(LD_DIR)/esp8266.project.ld.in -T$(LD_DIR)/../../esptool_py/esptool/flasher_stub/rom_8266.ld
LDFLAGS += -Xlinker --defsym -Xlinker otb_build_num=$$(cat $(BUILD_NUM_FILE))
RBOOT_LDFLAGS = -nostdlib -Wl,--no-check-sections -u call_user_start -Wl,-static  -T$(LD_DIR)/esp8266.rom.ld -T$(LD_DIR)/esp8266.peripherals.ld -T$(LD_DIR)/esp8266.project.ld.in -T$(LD_DIR)/../../esptool_py/esptool/flasher_stub/rom_8266.ld
...
# Object files
otbObjects = $(OTB_OBJ_DIR)/otb_main.o
#             $(OTB_OBJ_DIR)/otb_ds18b20.o \
...
bin/app_image.elf: build_num libphy2 otb_objects
#	$(LD) $(LDFLAGS) -o bin/app_image.elf $(otbObjects) $(httpdObjects) $(mqttObjects) $(i2cObjects) $(softuartObjects) $(LDLIBS)
	./gen_ld.sh $(LD) $(LD_MAIN_SCRIPT) bin/ld $(CFLAGS) -DAPP_OFFSET=0x8000 -DAPP_SIZE=0x74000
	$(LD) -Tbin/ld $(LDFLAGS) -o bin/app_image.elf $(otbObjects) $(LDLIBS)
